...
out["txt_label"] = {
    "input": row,
    "actual_xyxy": YoloDataset.load_valid_labels(None, "probe", [row]).tolist(),
    "expected_xyxy": [[0, 0.4, 0.4, 0.6, 0.6]],
}
...
out["auto_stride_bn"] = {
    "changed_buffers": len(changed),
    "examined_buffers": len(before),
    "sample": {
        k: {"before": before[k].flatten()[:3].tolist(), "after": after[k].flatten()[:3].tolist()} for k in changed[:3]
    },
}
...
for norm in [1, 4]:
    ...
    for _ in range(4):
        ClosureResult.from_training_step_output(p * 16, normalize=norm).closure_loss.backward()
    grads.append(float(p.grad))
out["accumulation_gradient"] = {"official_sum_example": grads[0], "lightning_default_example": grads[1]}
cb = GradientAccumulation(
    OmegaConf.create({"equivalent_batch_size": 64, "batch_size": 16}), OmegaConf.create({"warmup": {"epochs": 3}})
)
...
for epoch in range(10):
    ...
    for batch in range(100):
        ...
        if (batch + 1) % trainer.accumulate_grad_batches == 0 or batch == 99:
...
for a in data["annotations"]:
    if a["iscrowd"] or not a.get("segmentation") or not isinstance(a["segmentation"], list):
        continue
...
out["coco_val_annotations"] = {
    "images": len(data["images"]),
    "annotations": len(data["annotations"]),
    "crowd_excluded": sum(bool(a["iscrowd"]) for a in data["annotations"]),
    "noncrowd_images_gt_over100": sum(n > 100 for n in counts.values()),
    "noncrowd_gt_dropped_by_cap": sum(max(0, n - 100) for n in counts.values()),
    "polygon_vs_bbox_gt_compared": len(diffs),
    "polygon_bbox_diff_gt1px": sum(d > 1 for d in diffs),
    "polygon_bbox_maxdiff_px": max(diffs),
}
tiny = YOLO.__new__(YOLO)
torch.nn.Module.__init__(tiny)
tiny.model = torch.nn.Sequential(torch.nn.Linear(1, 1, bias=False))
tiny.save_load_weights(
    {"state_dict": {"model.model.0.weight": torch.tensor([[2.0]]), "ema.model.0.weight": torch.tensor([[7.0]])}}
)
...
out["provenance"] = {
    ...
    "scope": "CPU behavioral probes and COCO val annotation inspection; not AP reproduction",
}
