...
source_hashes = {
    str(p.relative_to(args.source_root.resolve())): hashlib.sha256(p.read_bytes()).hexdigest()
...
class Timing(Callback):
    ...
    def on_validation_start(self, trainer, module):
        if self.fixed_state is not None:
            ...
            assert all(
                torch.equal(value.cpu(), self.fixed_state[key]) for key, value in module.model.state_dict().items()
            )
        torch.cuda.synchronize()
...
def main():
    ...
    callbacks = (
        ...
        else [GradientAccumulation(cfg.task.data, cfg.task.scheduler), EMA(cfg.task.ema.decay), timing]
    )
    trainer = Trainer(
        accelerator="gpu",
        devices=1,
        ...
        **({"gradient_clip_val": 10, "gradient_clip_algorithm": "norm"} if module.automatic_optimization else {}),
    ...
    assert cache_hashes == {
        p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in sorted(args.dataset.glob("*.pache"))
    }, "Dataset cache changed during benchmark"
    result = {
        "source": yolo.__file__,
        "source_sha256": source_hashes,
        "dataset_cache_sha256": cache_hashes,
        ...
        "eval_state": str(args.eval_state.resolve()) if args.eval_state else None,
        "eval_state_sha256": eval_state_hash,
        "setup_modified_state_keys_restored": timing.setup_modified_state_keys,
        "train_images": timing.train_images,
        "val_images": timing.val_images,
        "train_seconds": timing.train_seconds,
        "validation_seconds": timing.val_seconds,
        "fit_or_validate_seconds": elapsed,
        "train_images_per_second": timing.train_images / timing.train_seconds if timing.train_images else None,
        "validation_images_per_second": timing.val_images / timing.val_seconds,
        "train_peak_allocated_bytes": getattr(timing, "train_peak", None),
        "val_peak_allocated_bytes": timing.val_peak,
        "losses": [float(x) for x in timing.losses],
        "finite_weights": True,
        "metrics": {k: float(v) for k, v in trainer.callback_metrics.items() if v.numel() == 1},
    }
    (args.output / "result.json").write_text(json.dumps(result, indent=2) + "\n")
    print(
        json.dumps({k: v for k, v in result.items() if k not in ("source_sha256", "losses", "metrics")}, indent=2),
        flush=True,
    )
