...
def main():
    ...
    parser.add_argument("--metric-cpu", action="store_true", help="Store metric states on CPU to avoid per-detection CUDA scalar copies")
    ...
    with initialize_config_dir(config_dir=str(args.source_root.resolve() / "yolo/config"), version_base=None):
        cfg = compose(config_name="config", overrides=["task=train", "model=v9-t", "dataset=coco-subset", "weight=false",
            "cpu_num=4", "image_size=[320,320]", "task.data.batch_size=32", "task.validation.data.batch_size=32", "+model.anchor.strides=[8,16,32]"])
    ...
    metric = MeanAveragePrecision(iou_type="bbox", box_format="xyxy", backend="faster_coco_eval", compute_on_cpu=args.metric_cpu).cuda()
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.float16):
        ...
        for index in range(len(loader)):
            ...
            images, targets = timed(val, "transfer", lambda: (images.cuda(non_blocking=True), targets.cuda(non_blocking=True)))
            ...
            formatted = timed(val, "metric_format", lambda: ([to_metrics_format(p) for p in preds], [to_metrics_format(t) for t in targets]))
            timed(val, "metric_update", lambda: metric.update(*formatted))
            images_seen += batch[0]
        metrics = timed(val, "metric_compute", metric.compute)
    result = {"gpu": torch.cuda.get_device_name(), "torch": torch.__version__, "val_images": images_seen, "metric_cpu": args.metric_cpu,
              "state_sha256": hashlib.sha256(args.state.read_bytes()).hexdigest(), "validation_stage_seconds": dict(val),
              "nms_candidates_per_batch": candidates, "map": float(metrics["map"])}
    if args.train_batches:
        ...
        for step in range(args.train_batches + 5):
            ...
            _, images, targets, *_ = batch
            images, targets = timed(current, "transfer", lambda: (images.cuda(non_blocking=True), targets.cuda(non_blocking=True)))
