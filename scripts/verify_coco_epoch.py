...
class Audit(Callback):
    ...
    def on_train_batch_end(self, trainer, module, outputs, batch, batch_idx):
        ...
        if self.batches % 100 == 0 or self.batches == 1:
            print(
                f"TRAIN {self.batches}/{trainer.num_training_batches} batches, {self.images} images, loss={loss:.5f}, elapsed={time.monotonic() - self.started:.0f}s",
                flush=True,
            )
...
def main():
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="20 training and two validation batches using val2017; not a full COCO epoch",
    )
    ...
    result = {
        "status": "passed",
        "full_coco_epoch": not args.smoke,
        "model": "v9-t",
        "pretrained": False,
        ...
        "train_images": audit.images,
        "peak_cuda_memory_allocated_bytes": torch.cuda.max_memory_allocated(),
        "train_batches": audit.batches,
        "validation_images": audit.validation_images,
        "completed_epochs": trainer.current_epoch,
        "optimizer_steps": trainer.global_step,
        "optimizer_step_attempts": trainer.global_step,
        "successful_optimizer_steps": ema.step,
        "amp_skipped_steps": trainer.global_step - ema.step,
        "finite_losses": True,
        "weights_changed": changed,
        "first_loss": audit.losses[0],
        "last_loss": audit.losses[-1],
