...
def load_reference(path):
    ...
    namespace = {
        "np": np, "cv2": cv2, "torch": torch, "math": math, "random": random,
    ...
    selections = {
        "utils/general.py": (
            "xywhn2xyxy", "xyxy2xywhn", "xyxy2xywh", "xyn2xy", "clip_boxes",
            "resample_segments", "segment2box",
        ),
        "utils/metrics.py": ("bbox_ioa",),
        "utils/augmentations.py": (
            "augment_hsv", "letterbox", "random_perspective", "copy_paste", "mixup", "box_candidates", "Albumentations",
        ),
    ...
    for filename, names in selections.items():
        source = subprocess.check_output(
            ["git", "-C", str(path), "show", f"{REFERENCE_COMMIT}:{filename}"]
        )
...
def check_case(name, reference_call, implementation_call, seeds, tolerances):
    ...
    for seed in range(seeds):
        ...
        components = [
            compare_arrays(left, right, tolerance)
            for left, right, tolerance in zip(expected, actual, tolerances)
        ]
    ...
    return {
        "name": name, "passed": not failures, "maximum_abs_errors": maximums,
        "tolerances": list(tolerances), "seeds": seeds, "failed_seeds": failures,
    }
...
def run_checks(reference, seeds=32):
        ...
        for perspective in (0.0, 0.0005):
            ...
            def expected_geometry():
                return reference["random_perspective"](
                    image.copy(), labels.copy(), copy.deepcopy(selected_segments),
                    degrees=0, translate=0.1, scale=0.9, shear=0,
                    perspective=perspective, border=(-32, -32),
                )

            def actual_geometry():
                aligned = selected_segments or [np.zeros((0, 2), dtype=np.float32) for _ in labels]
                return implementation._random_perspective(
                    image.copy(), labels.copy(), copy.deepcopy(aligned),
                    degrees=0, translate=0.1, scale_gain=0.9, shear=0,
                    perspective=perspective, border=(-32, -32),
                )
    ...
    for probability in (0.3, 1.0):
        def expected_copy():
            result = reference["copy_paste"](
                image.copy(), labels.copy(), copy.deepcopy(segments), p=probability
            )
    ...
    with tempfile.TemporaryDirectory(prefix="yolov9-augmentation-parity-") as directory:
        ...
        for index, (height, width) in enumerate(((192, 256), (241, 131), (113, 229), (171, 171))):
            ...
            checks.append(check_case(
                f"resize_{height}x{width}", lambda: (dataset.load_image(index)[0],),
                lambda: (implementation._resize_longest(source, 128),), 1, (0,),
            ))
...
def check_pipelines(reference, implementation, dataset, source_images, seeds, albumentations):
    ...
    dataset.mosaic = True
    dataset.rect = False
    dataset.albumentations = reference["Albumentations"](128) if albumentations else lambda image, labels: (image, labels)
    ...
    source_segments = [
        ...
        for x1, y1, x2, y2 in normal_xyxy
    ]
    ...
    for label_mode in ("boxes", "segments", "empty"):
        ...
        for mosaic, mixup in ((0.0, 0.0), (1.0, 0.0), (1.0, 1.0), (1.0, 0.15)):
            augmentation = implementation.YOLOv9Augmentation(128, mosaic=mosaic, mixup=mixup, albumentations=albumentations)
            ...
            def actual_pipeline():
                def raw_sample(index):
                    return source_images[index][:, :, ::-1].copy(), selected_boxes.copy(), copy.deepcopy(source_segments) if use_segments else []
                ...
                output_image, output_labels, _ = augmentation(
                    source_images[0][:, :, ::-1].copy(), selected_boxes.copy(),
                    copy.deepcopy(source_segments) if use_segments else [], sample,
                )
...
def main():
    ...
    try:
        ...
        report = {
            ...
            "versions": {"opencv": cv2.__version__, "numpy": np.__version__, "torch": torch.__version__, "albumentations": version("albumentations")},
