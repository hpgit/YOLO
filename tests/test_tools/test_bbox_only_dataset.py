...
def _write_dataset(root, source="txt", annotation_extra=None):
    ...
    if source in {"txt", "split"}:
        ...
        annotations = root / "annotations"
        annotations.mkdir()
        (annotations / "instances_sample.json").write_text(
            json.dumps(
                {
                    "images": [{"id": i, "file_name": f"{i}.png", "width": 64, "height": 32} for i in range(2)],
                    "categories": [{"id": 7, "name": "object"}],
                    "annotations": [
                        dict(id=i + 1, image_id=i, category_id=7, bbox=[8, 8, 32, 16], **(annotation_extra or {}))
                        for i in range(2)
                    ],
                }
            )
...
def test_bbox_only_coordinates_survive_train_and_validation(tmp_path, source, reference):
    ...
    expected = torch.tensor([[[0.0, 8.0, 24.0, 40.0, 40.0]]] * 2)
    ...
    assert float(result["map"]) == pytest.approx(1.0)
    assert float(result["mar_100"]) == pytest.approx(1.0)
...
def test_coco_bbox_takes_precedence_over_optional_segmentation(tmp_path, extra):
    ...
    for reference in (False, True):
        ...
        np.testing.assert_allclose(loader.dataset.bboxes[0], [[0, 0.125, 0.25, 0.625, 0.75]])
...
def test_mixed_labels_empty_images_and_partial_boxes(tmp_path, reference):
    _write_dataset(tmp_path)
    (tmp_path / "labels/sample/0.txt").write_text("0 0.0 0.5 0.5 0.5\n0 0.125 0.25 0.625 0.25 0.625 0.75 0.125 0.75\n")
    (tmp_path / "labels/sample/1.txt").unlink()
    data, dataset = _configs(tmp_path, reference)
    batch = next(iter(create_dataloader(data, dataset, "train")))
    torch.testing.assert_close(batch[2][0], torch.tensor([[0.0, 0.0, 24.0, 16.0, 40.0], [0.0, 8.0, 24.0, 40.0, 40.0]]))
...
def test_invalid_coco_boxes_are_skipped_in_both_loaders(tmp_path, bbox):
    ...
    for reference in (False, True):
        ...
        np.testing.assert_allclose(loader.dataset.bboxes[1], [[0, 0.125, 0.25, 0.625, 0.75]])


@pytest.mark.parametrize(
    "bbox, expected",
    [
        ([-8, 8, 24, 40], [0, 0.0, 0.25, 0.25, 1.0]),
        ([63, 31, 0.0001, 0.0001], [0, 63 / 64, 31 / 32, 63.0001 / 64, 31.0001 / 32]),
    ],
)
...
def test_old_bbox_cache_is_rebuilt_then_reused_and_class_config_revalidated(tmp_path, monkeypatch):
    ...
    torch.save([(tmp_path / "images/sample/0.png", torch.zeros(1, 5), 1.0)], cache)
    data, dataset = _configs(tmp_path)
    first = create_dataloader(data, dataset).dataset
    np.testing.assert_allclose(first.bboxes[0], [[0, 0.125, 0.25, 0.625, 0.75]])
    assert len(first) == 2
...
def test_bbox_only_coco_defaults_preserve_supplied_metadata_and_source(tmp_path):
    _write_dataset(tmp_path, "json", {"area": 123.0, "iscrowd": 1})
    path = tmp_path / "annotations/instances_sample.json"
    before = path.read_bytes()
    metric = CocoJsonEvaluator(path)
    assert all(a["area"] == 123.0 and a["iscrowd"] == 1 for a in metric.coco_gt.anns.values())
