...
class StreamDataLoader:
    """Single-pass, unsized input: consumers must iterate until actual EOF.

    Prefetched queue occupancy is not a length, and video/live frame counts may
    be unknown. In particular, exposing it as __len__ truncates Trainer.predict.
    """
    ...
    def __init__(self, data_cfg: DataConfig):
        ...
        if self.is_stream:
            ...
            self.thread = Thread(target=self.load_source, daemon=True)
            self.thread.start()
    ...
    def load_source(self):
        try:
            ...
            elif self.source.suffix.lower() in [".mp4", ".avi", ".mkv"]:
    ...
    def process_frame(self, frame):
        ...
        if not self.is_stream:
            while not self.stop_event.is_set():
                try:
                    self.queue.put((frame, rev_tensor, origin_frame), timeout=0.1)
                    break
                except Full:
                    continue
        else:
    ...
    def __next__(self) -> Tensor:
        ...
        if self.is_stream:
            ...
            while not self.stop_event.is_set():
                # Queue occupancy is neither the input length nor an EOF signal.
                # Check completion before get so a final enqueue cannot be lost.
                done = self.source_done.is_set()
                try:
